...
import com.example.mobiletpa.adapters.ViewOrderAdapter
...
class ViewOrderActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        recyclerView = findViewById(R.id.vieworderRv)
        ...
        val adapter = ViewOrderAdapter(orderList, this)
            ...
            override fun onDataChange(p0: DataSnapshot) {
                ...
                for (dt in p0.children) {
                    if (dt.key.toString().contains(mAuth.currentUser?.uid.toString())) {
                        if (dt.getValue(Order::class.java)?.status.toString().equals("") == true) {
                            var data = Order()
